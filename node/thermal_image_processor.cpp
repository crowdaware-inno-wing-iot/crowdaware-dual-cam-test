...
void convert_to_8bit_image(const float* thermal_frame, uint8_t image[IMAGE_HEIGHT][IMAGE_WIDTH]) {
    for (int i = 0; i < 768; i++) {
        ...
        // MLX90640 is 32x24 (IMAGE_WIDTH x IMAGE_HEIGHT)
        int y = i / IMAGE_WIDTH;
        int x = i % IMAGE_WIDTH;
...
void distance_transform(const uint8_t input[IMAGE_HEIGHT][IMAGE_WIDTH], uint8_t output[IMAGE_HEIGHT][IMAGE_WIDTH]) {
    ...
    for (int y = 0; y < IMAGE_HEIGHT; y++) {
        for (int x = 0; x < IMAGE_WIDTH; x++) {
            output[y][x] = (input[y][x] > DT_BG_THRESHOLD) ? DT_MAX_DISTANCE : 0;
        }
    }
...
static uint8_t eroded_buf[IMAGE_HEIGHT][IMAGE_WIDTH];
static uint8_t blurred_buf[IMAGE_HEIGHT][IMAGE_WIDTH];

static int stack_y[IMAGE_SIZE];
static int stack_x[IMAGE_SIZE];
...
uint8_t process_thermal_frame(ThermalProcessor* processor,
    ...
    uint8_t num_people = 0;
    const size_t bytes = IMAGE_HEIGHT * IMAGE_WIDTH * sizeof(uint8_t);
    ...
    erode_3x3(processor->work_buffer, eroded_buf);
    dilate_3x3(eroded_buf, processor->work_buffer);

    // Step 3: Gaussian blur for preprocessing
    gaussian_blur_3x3(processor->work_buffer, blurred_buf);

    // Step 4: Distance transform
    distance_transform(blurred_buf, processor->distance_map);
